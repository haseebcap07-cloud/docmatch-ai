...
class AdaptiveResumeEngine:
    def tailor(self,profile:ResumeProfile,job_description:str,target_role:str,custom_instructions:str,template:TemplateSettings,user_requested_additions:list[str]|None=None,user_requested_replacements:list[str]|None=None)->dict:
        ...
        profile_text=profile_to_text(profile); source_layout=profile.source_layout or {}; preserve_structure=bool(getattr(template,'preserve_source_structure',True))
        ...
        if provider=='openai' and settings.OPENAI_API_KEY:
            try: ai=self._openai_tailor(profile,job_description,target_role,custom_instructions,template,baseline_breakdown.model_dump(),match,resume_class,jd_class,role_alignment,playbook,jd_analysis,evidence,semantic_result,user_requested_additions,user_requested_replacements,source_layout,preserve_structure,exp_matrix)
                ...
                ai=self._rule_tailor(profile,target_role,match,playbook,evidence,role_alignment,jd_analysis,semantic_result,user_requested_additions,exp_matrix); ai['recruiter_warnings'].insert(0,f'OpenAI failed; V8 fallback used: {str(exc)[:180]}')
        ...
        ai['score_reason']=str(ai.get('score_reason') or self._score_reason(role_alignment)).strip(); validation=validate_generated_resume(ai,evidence,profile_text); ai['recruiter_warnings'].extend(validation.get('validator_warnings',[]))
        ...
        change_log=build_change_log(semantic_result,match,user_requested_additions,user_requested_replacements,evidence,f'V8 preserved uploaded structure where requested; original employer titles, names, and dates preserved.')
...
STRUCTURE: preserve_source_structure={preserve_structure}; source layout={json.dumps(source_layout,indent=2)}.
